...
########################################
#
# structure used to count occurrences
#
########################################
class OccurrenceCounter:
	...
	def addOccurrence(self, pOccurrence):
		if not pOccurrence in self.mData:
			self.mData[pOccurrence] = 0
		self.mData[pOccurrence] = self.mData[pOccurrence] + 1
...
class SessionAggregator:
	...
	def parseReport(self):
		...
		lEntriesPerType = OccurrenceCounter()
		lRedirectUris = OccurrenceCounter()
		lDataSessionOidcSubs = OccurrenceCounter()
		lDataSessionProxyIds = OccurrenceCounter()
		...
		with open(self.__mActivityReport) as lF:
			...
			for r in lF:
				...
				try:
					...
					lEntriesPerType.addOccurrence(lType)
					
					
					## DIRECTORY SESSION
					if lType in ['DirectoryApiStatus','AdminRegistrationCodeDelivered']:
						pass
					elif lType in ['Admin','HomePage','Client']:
						...
						if lSession is not None and lResult == 'E_No_Error':
							...
							lRedirectUris.addOccurrence(lJson['redirecturi'])
							
					elif lType in ['RequestDirectorySessionAuthBis','RequestDirectorySessionAuth', 'RequestRegisterFrontEndUser']:
						lSession = self.__editDirectorySession(lJson)
					...
					#			self.__closePendingDataSessions('service stop',lTs,lJson['proxyid'])
						...
						if not 'oidcsub' in lJson:
							...
							lDataSessionOidcSubs.addOccurrence(lJson['oidcsub'])
						if 'proxyurl' in lJson:
							lDataSessionProxyIds.addOccurrence(lJson['proxyurl'])
						...
						# directorysessionid was missing from log entries in some version of 4.0.x
						lDirectorySessionId = lJson['directorysessionid'] if 'directorysessionid' in lJson else None
	...
	def processEntries(self, pEntries):
		...
		for e in pEntries:
			...
			if e['type'] == 'borrowcountchange':
				...
				if e['revokeinfo'].lower() in ['close by application','directory session was removed','revoked by admin','end of life','user was removed or disabled','end of report', 'build or proxy is no more available'] : # 'Close by application','Revoked by the directory','revoked by proxy event','Lost directory connection','end of life','pg_role was destroyed','Service is about to exit', 'Build or Proxy is no more available'
					lEndTs = e['endts']
				...
				if 'directorysessionid' in e and not e['directorysessionid'] in lDirectorySessions:
		...
		jsonObjectListToCsv(lNewEntries,['id','tsstr','type','endtsstr','revokeinfo','lic_count_delta','all_lic_count_cumulated','only_borrow_count_cumulated'], ['activesessions'] if self.__mOutputActiveSessions else [],self.__mOutput,self.__mCsvDelimiter)
...
if __name__ == '__main__':
	...
	for p in ['./bin','../../GENERATED/DELIVERY/MSVC17_x64/Tool_ValidateActivityReport/bin']:
